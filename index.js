...
function displayTasks() {
  ...
  for (let i = 0; i <= Number(localStorage.getItem("task-count")); i++) {
    ...
    if (localStorage.getItem("task" + i.toString()) != null) {
        ...
        if (event.key === "Enter") {
          ...
          if (user.value.length > 0) {
            let task_count = Number(
              localStorage.getItem("task-count" + i.toString())
            );
            task_count += 1;
            ...
            localStorage.setItem(
              "task" + i.toString() + task_count.toString(),
              user.value
            );
            user.value = "";
            displayTasks();
      ...
      for (
        let j = 0;
        j <= Number(localStorage.getItem("task-count" + i.toString()));
        j++
      ) {
        console.log(localStorage.getItem("task" + i.toString() + j.toString()));
        if (
          localStorage.getItem("task" + i.toString() + j.toString()) != null
        ) {
          const task = localStorage.getItem(
            "task" + i.toString() + j.toString()
          );
          const task_div = document.createElement("div");
          const content_paragraph = document.createElement("p");
          task_div.className = "task-div";
